...
AVAILABLE_TOOLS: List[ToolDefinition] = [
    ...
    ToolDefinition(
        name="gaode_geo",
        description="查询地点的经纬度坐标。当需要获取地点的精确位置用于路线规划时使用。返回格式为'纬度,经度'。",
        parameters={
            "type": "object",
            "properties": {
                "address": {
                    "type": "string",
                    "description": "地点名称或地址，例如：'南京大学'、'杭州西湖'"
                },
                "city": {
                    "type": "string",
                    "description": "城市名称，用于提高搜索准确性，例如：'南京'、'杭州'"
                }
            },
            "required": ["address"]
        },
    ...
    ToolDefinition(
        name="gaode_transit",
        description="查询公共交通路线规划（地铁+公交），包括详细的换乘方案、站点信息、票价和预计时间。当用户需要查询城市内或城市间的公共交通路线时使用。",
        parameters={
            "type": "object",
            "properties": {
                "origin": {
                    "type": "string",
                    "description": "起点坐标（纬度,经度），格式：'纬度,经度'，例如：'31.920763,118.792331'。必须先使用gaode_geo获取坐标。"
                },
                "destination": {
                    "type": "string",
                    "description": "终点坐标（纬度,经度），格式：'纬度,经度'，例如：'32.113574,118.916507'。必须先使用gaode_geo获取坐标。"
                },
                "city": {
                    "type": "string",
                    "description": "城市名称，用于限定搜索范围，例如：'南京'、'上海'"
                }
            },
            "required": ["origin", "destination"]
        },
